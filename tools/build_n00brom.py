#!/usr/bin/env python3
"""Build n00bROM's native cartridge image and legacy relocated PS-X EXE."""
...
def build(source_dir: Path, armips: Path, out_dir: Path) -> dict[str, str]:
    ...
    with tempfile.TemporaryDirectory(prefix="n00brom-build-") as temp:
        ...
        # This is retained solely as a source-compatibility reference. The
        # actual standalone release is built by build_baremetal.py and owns
        # the n00brom.psexe name, avoiding accidental distribution of the
        # BIOS-hook/cartridge-derived implementation.
        psexe = out_dir / "n00brom-relocated-legacy.psexe"
        write_psexe(
            raw.read_bytes(),
            psexe,
    ...
    manifest = {
        ...
        "n00brom_relocated_legacy_psexe_sha256": sha256(psexe),
        "n00brom_rom_size": str(native.stat().st_size),
        "n00brom_relocated_legacy_psexe_size": str(psexe.stat().st_size),
    }
